...
def extract_positive_features(data_file,LM,w2v_model,d2v_model,google_model,pos_label,features):
    ...
    all_pos_features= np.array([]).reshape(0,len(features))
    for i in range(len(f)):
        ...
        #print (passage)
        pos_features=extract_features(passage,LM,w2v_model,d2v_model,google_model,pos_label)
        if pos_features==[]:
            continue
        #print (pos_features)
...
def main():
    ...
    feat_pos=extract_positive_features(data_file,LM,w2v_model,d2v_model,google_model,pos_label,features)
